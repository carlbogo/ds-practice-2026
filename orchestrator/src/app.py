...
books_db_grpc_path = os.path.abspath(
    os.path.join(FILE, "../../../utils/pb/books_database")
)
sys.path.insert(0, books_db_grpc_path)

import books_database_pb2 as books_pb2
import books_database_pb2_grpc as books_pb2_grpc
...
def checkout():
    ...
    # }
    # Assume all orders are valid
    logger.info("event=order_received cid=%s", correlation_id)

    try:
        enqueued = _enqueue_order(request_data, [], correlation_id)
        if enqueued:
            logger.info("event=order_enqueued cid=%s", correlation_id)
        else:
            logger.error("event=enqueue_failed cid=%s", correlation_id)
            return {
                "status": "Order Failed",
                "reasons": ["Failed to enqueue order"],
                "suggestedBooks": [],
            }, 500
    except grpc.RpcError as error:
        logger.error(
        ...
        return {
            "status": "Order Failed",
            "reasons": ["Order queue unavailable"],
            "suggestedBooks": [],
        }, 503
    ...
    try:
        response = books_db_stub.ListAvailableBooks(books_pb2.Empty())
        available_books = list(response.books)
        purchased = {
            item.get("name", "")
            for item in request_data.get("items", [])
        }
        available_books = [
            b for b in available_books if b not in purchased
        ]
